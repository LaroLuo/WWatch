...
sched_sleep_on(ktqueue_t *q)
{
	...
	dbg ( DBG_PRINT, "\n\n GRADING1C 1 Faber Test : waitpid any test" );

	KASSERT ( curthr != NULL && q != NULL );
...
sched_cancellable_sleep_on(ktqueue_t *q)
{
	...
	if(curthr->kt_cancelled == 1)
	{
		...
		dbg ( DBG_PRINT, "\n\n GRADING1C 5 Faber Test : cancel me test" );
	...
	dbg ( DBG_PRINT, "\n\n GRADING1C 3 Faber Test : wake me test" );
...
sched_wakeup_on(ktqueue_t *q)
{
	...
	if( ! sched_queue_empty (q) )
	{
		...
		dbg ( DBG_PRINT, "\n\n GRADING1E Self-check in sched_wakeup_on when sched_queue not empty");

		kthread_t *tmp = ktqueue_dequeue(q);

		KASSERT ( tmp != NULL );

		/* Checking for thread state and KASSERT use as mentioned in Grading Guideline 4.(a)*/
		KASSERT ( tmp->kt_state == KT_SLEEP || tmp->kt_state == KT_SLEEP_CANCELLABLE );
		dbg ( DBG_PRINT, "\n\n  GRADING1A 4.a KASSERT ( tmp->kt_state == KT_SLEEP || tmp->kt_state == KT_SLEEP_CANCELLABLE ) is checking successfully." );

		sched_make_runnable(tmp);
	...
	dbg ( DBG_PRINT, "\n\n GRADING1C 6 Faber Test : reparenting test" );
...
sched_broadcast_on(ktqueue_t *q)
{
	...
	while ( ! sched_queue_empty (q) )
	{
		...
		dbg ( DBG_PRINT, "\n\n GRADING1E Self-check in sched_broadcast when sched_queue not empty");
		sched_wakeup_on(q);
	}
...
sched_switch(void)
{
	...
	while ( sched_queue_empty ( &kt_runq ) )
	{
		...
		dbg ( DBG_PRINT, "\n\n  GRADING1E Self-check in sched_switch when sched_queue empty " );
		/* dbg ( DBG_PRINT, "\n\n  GRADING1D 2 Sunghan Deadlock Test " ); */
		intr_disable();
		intr_setipl ( IPL_LOW );
		intr_wait();
	}
...
sched_make_runnable(kthread_t *thr)
{
	...
	/* dbg ( DBG_PRINT, "\n\n  GRADING1E Self-check in sched_make_runnable" ); */

	KASSERT ( thr != NULL );

	KASSERT ( &kt_runq != thr->kt_wchan );
	dbg ( DBG_PRINT, "\n\n  GRADING1A 4.b KASSERT ( &kt_runq != thr->kt_wchan ) is checking successfully." );
